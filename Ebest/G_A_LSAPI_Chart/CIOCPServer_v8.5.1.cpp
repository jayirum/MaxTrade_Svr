...
bool CIOCPServer::Start(char* ip, uint16_t port, int acceptDepth, int nWorkers)
{
    ...
    __common.log_fmt(INFO, "bind port:%d", port);

    if (::bind(m_sock_listen, (sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) { 
        perrorW("bind"); 
        return false; 
    }
...
void CIOCPServer::perrorW(const char* msg) {
    ...
    __common.log_fmt(ERR, "[%s](%d:%s)", msg, err, (char*)lpMsgBuf);
    LocalFree(lpMsgBuf);
}
...
//====================== 세션 관리(COW) ======================
void CIOCPServer::Session_AddToList(const SessionPtr& newSession)
{
    for (;;)
    {
        ...
        */
        ...
        if (is_snapped_non_changed){
            __common.log_fmt(INFO, "[Accept Client](Total:%d)(Client-Idx:%d)(socket:%s)", size+1, size, newSession->m_sock_s);
            break;
        }
    }

    //===== 이미 close 한 세션 최종 정리
    Session_DestroyClosedSession();
}


void CIOCPServer::Session_DestroyClosedSession()
{
    std::lock_guard<mutex> lock(m_mtxClosedSess);

    for (auto it = m_ClosedSessions.begin(); it != m_ClosedSessions.end(); )
    {
        SessionPtr p = *it;
        if (p->m_close_guard.use_count() == 0 && p->m_state == State::Closed)
        {
            //__common.log_fmt(INFO, "[Close-Destroy socket(%s), my_idx(%d)", p->m_sock_s, p->m_my_idx);
            
            it = m_ClosedSessions.erase(it);
        }
    }
}

void CIOCPServer::Session_RemoveFromList(const SessionPtr& target)
{
    if(!target) return;

    // 1) 우선 세션 종료 트리거 (중복 호출 안전)
    char sock_id[32]; sprintf(sock_id, "%d",target->m_sock);
    bool is_continue = target->CloseSession_Request();  // 상태 Open->Closing, CancelIoEx 실행
    if(!is_continue) return;    // 이미 closing or closed; 아래 로직 수행할 필요 없다.

    // 이 아래는 최초 한번 실행되므로 이때 shared_ptr 백업한다.
    {
        std::lock_guard<mutex> lock(m_mtxClosedSess);
        m_ClosedSessions.push_back(target);
    }

    //__common.log_fmt(INFO, "[Close-RemoveSession](socket:%s)(idx:%d)", target->m_sock_s, target->m_my_idx);
    ...
    for (;;)
    {
        ...
        // target이 없는 경우 빠른 탈출
        if (std::none_of(oldVec->begin(), oldVec->end(),
            [&](const SessionPtr& sp) { return sp.get() == target.get(); }))
            break;

        auto newVec = std::make_shared<std::vector<SessionPtr>>();
        newVec->reserve(oldVec->size());
        for (auto& sp : *oldVec){
            if (sp.get() != target.get())
                newVec->emplace_back(sp);
        }

        // 변경대상(m_Sessions) 이 expected(oldVec) 과 같으면 desired(newVec) 으로 변경하고, return true;
        bool is_snapped_non_changed = std::atomic_compare_exchange_weak(&m_Sessions, &oldVec, newVec);
        if (is_snapped_non_changed)
        {
            //int size = (newVec && !newVec->empty()) ? static_cast<int>(newVec->size()) : 0;
            //__common.log_fmt(INFO, "[Session_RemoveFromList](Total Size:%d)(Client-Idx:%d)(Socket:%s)", 
            //                    size, target->m_my_idx, sock_id);
    ...
    target->CloseSession_Finalize(); // in-flight==0이면 즉시 닫힘
}
...
void CIOCPServer::WorkerLoop()
{
    for (;;)
    {
        ...
        if (!ok && pOvlap) { bytes = 0; }                           // bytes는 미정의 → 반드시 무시/0으로 세팅
        if (!ok && err== ERROR_OPERATION_ABORTED) { bytes = 0; }    // CancelIoEx에 의해 취소된 IO
        ...
        case TOvlapBase::Type::Accept:
        {
            ...
            sess->m_recvCtx.self    = sess;
            sprintf(sess->m_sock_s, "%d", sock);

            Session_AddToList(sess);
        ...
        case TOvlapBase::Type::Recv: {
            auto ctx    = static_cast<TRecvCtx*>(base);
            auto sess   = ctx->self.lock();  // shared_ptr 의 참조카운트 증가로 당장 삭제 되는걸 방지. OnRecvComplete 가 끝날때 까지 유지
            if (sess) {
                sess->OnRecvCompleted(bytes, err);
            }
            break;
        }
        case TOvlapBase::Type::Send: {
            auto* ctx = static_cast<TSendCtx*>(base);
            auto sess = ctx->selfSessionPtr;            // shared_ptr 의 참조카운트 증가
            sess->OnSendCompleted(ctx, bytes, err);
            break;
        }
        }
    }
...
void CIOCPServer::TSession::OnRecvCompleted(DWORD bytes, DWORD wsagetlasterror)
{
    dec_io_inflight();

    // CancelIoEx로 인해 취소된 IO → 아무 것도 안 하고 종료
    if (wsagetlasterror == ERROR_OPERATION_ABORTED) { return;}
    
    /* peer closed */
    if (bytes == 0 || wsagetlasterror!=0) {
        m_iocp_ptr->Session_RemoveFromList(shared_from_this());
        return; 
    }
...
{
    //===== closing 이 진행중인 경우 수행하지 않는다 =====//
    if( m_state.load()!=State::Open ) return;
    ...
    {
        dec_io_inflight();  // 발행 취소 보정

        //m_send_ringQ.set_pop_available();

        __common.log_fmt(ERR, "[PostSend Error-sock:%d](%d)", m_sock, WSAGetLastError());
...
void CIOCPServer::TSession::OnSendCompleted(TSendCtx* ctx, DWORD /*bytes*/, DWORD wsagetlasterror)
{
    dec_io_inflight();
...
{
    //===== closing 이 진행중인 경우 수행하지 않는다 =====//
    if(sess->m_state.load()!=State::Open) return false;

    m_recvCtx.self = shared_from_this();
...
void CIOCPServer::TSession::CloseSession_TryFinalize() {
    if (m_state.load(std::memory_order_acquire) == State::Closing &&
        m_io_inflight.load(std::memory_order_acquire) == 0)
    {
        CloseSession_Finalize();
    }
}
void CIOCPServer::TSession::CloseSession_Finalize() {
    if (m_sock != INVALID_SOCKET) {
        ::closesocket(m_sock);
        m_sock = INVALID_SOCKET;
    }
    m_state.store(State::Closed, std::memory_order_release);
    m_close_guard.reset(); // 생존 보장 핸들 해제 → 이 시점에 참조가 0이면 객체 파괴

    //__common.log_fmt(INFO, "[Close-Finalize](socket:%s)(idx:%d)", m_sock_s, m_my_idx);
}

// === 종료 절차 엔트리 ===
bool CIOCPServer::TSession::CloseSession_Request()
{
    State expected = State::Open;
    if (!m_state.compare_exchange_strong(
            expected, 
            State::Closing,
            std::memory_order_acq_rel,  // 성공시. CAS
            std::memory_order_acquire   // 실패시. READ
            )) 
        return false; //값이 expected 와 다를 때 false 반환. 이미 Closing/Closed

    // 닫기 동안 자기 자신을 붙들어 둠 (컨테이너에서 빠져도 생존)
    m_close_guard = shared_from_this();

    if (m_sock != INVALID_SOCKET) {
        ::shutdown(m_sock, SD_BOTH);               // 그레이스풀
        ::CancelIoEx((HANDLE)m_sock, nullptr);     // 보류 IO 취소
    }

    //__common.log_fmt(INFO, "[Close-Request](socket:%s)(idx:%d)", m_sock_s, m_my_idx);
