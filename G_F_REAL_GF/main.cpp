...
int  _Start()
{
	...
	*/
	...
	if (!gSenders.Initialize(atoi(val))) {
		gCommon.log(INFO, FALSE, "Failed to Initialize CSenderList");
		return 0;
	}
	gCommon.log(INFO, TRUE, "2.CSenderList Initialized successfully");
	std::this_thread::sleep_for(std::chrono::seconds(1));
	...
	*/
	...
	acceptor.Initialize();
	gCommon.log(INFO, TRUE, "3.CAcceptor Initialized successfully.ListenPort:%s", zPort);
	...
	*/
	...
	gCommon.log(INFO, TRUE, "4.CAPIClient Connected successfully");

	
	DWORD ret = WaitForSingleObject(g_hDieEvent, INFINITE);
	
	acceptor.DeIitialize();
	//gSenders.DeInitialize();
	api.DeInitialize();
