...
void CAPIClient::threadFunc_recv_apiData()
{
	__try
	{
		threadFunc_recv_apiData_Internal();
	}
	__except (EXCEPTION_EXECUTE_HANDLER)
	{
		gCommon.log(ERR, FALSE, "---------------------------------------------------");
		gCommon.log(ERR, TRUE, "unexpected exception in CAPIClient::threadFunc_recv_apiData");
		gCommon.log(ERR, FALSE, "---------------------------------------------------");
	}
}

void CAPIClient::threadFunc_recv_apiData_Internal()
{
	char recvBuff[1024];
...
void CAPIClient::threadfunc_parse_deploy()
{
	__try
	{
		threadfunc_parse_deploy_Internal();
	}
	__except (EXCEPTION_EXECUTE_HANDLER)
	{
		gCommon.log(ERR, FALSE, "---------------------------------------------------");
		gCommon.log(ERR, TRUE, "unexpected exception in CAPIClient::threadfunc_parse_deploy");
		gCommon.log(ERR, FALSE, "---------------------------------------------------");
	}
}

void CAPIClient::threadfunc_parse_deploy_Internal()
{
	while (!m_thrdFlag.is_stopped())
	{
		std::this_thread::sleep_for(std::chrono::milliseconds(1));
		...
		if (!m_parsingBuffer.empty())
		{
			...
			{
				string send_buff = m_parsingBuffer.substr(0, pos);
				m_parsingBuffer.erase(0, pos + 1);
			...
			//{
			//	send_buff += m_parsingBuffer.substr(0, pos);	
			//	m_parsingBuffer.erase(0, pos + 1);
