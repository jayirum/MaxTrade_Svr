...
constexpr size_t CAP_SIZE = 4096;
...
struct SendTask : OVERLAPPED 
{
    ...
    Payload payload;
    SendTask(std::shared_ptr<Session> s, Payload p)
        : self(std::move(s)), payload(std::move(p)) 
    {
...
struct Session : std::enable_shared_from_this<Session> 
{
    ...
    void enqueue_send(const Payload& p) {
        ...
        PostQueuedCompletionStatus(g_iocp, 0, kStrandKey,reinterpret_cast<LPOVERLAPPED>(task));
    }
    // IOCP 워커 안 (strand)
    ...
    void on_recv_completed(DWORD bytes) {
        ...
        if (recvAcc.size() > kMaxLine) {
            remove_session(shared_from_this()); close_clientsock(s); return;
        }
...
void iocp_worker() {
    for (;;) {
        ...
        case OvType::Send: {
            auto* ctx = static_cast<SendCtx*>(shared_from_this(),base);
