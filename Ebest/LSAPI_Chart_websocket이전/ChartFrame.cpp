...
void CChartFrame::SetIndexName(int iType, LPCTSTR pszName)
{
	// IndexView를 생성합니다
    CView *pView = NULL;
    pView = (CView *)RUNTIME_CLASS(CChartAPIView)->CreateObject();
	ASSERT(pView);
    pView->Create(NULL, NULL, WS_CHILD | WS_VISIBLE, CRect( 0, 0, 0, 0 ), this, 0, NULL);

	// 생성된 IndexView를 초기화합니다
    CChartAPIView * pChartView = ( CChartAPIView* )pView;
    //JAY pChartView->SetIndexName(iType, pszName );
    pChartView->OnInitialUpdate();
