...
BOOL CLSAPIApp::InitInstance()
{
	...
	// 적절한 내용으로 수정해야 합니다.
	SetRegistryKey(_T(DEF_EXENAME));


	//==================== 프로그램 각종 초기화 작업 ====================//

	//===== CGlobals 초기화 (directory, log 등)
	...
	if(!__common.read_config_all() ) {
		...
		__common.log(ERR, "__common.read_config_all() error");
		return FALSE;
	}
...
bool CLSAPIApp::load_symbols()
{
	if (!__dbworks.is_connected())
		return false;
		
	__common.symbols = __dbworks.load_symbols();
	if (__common.symbols.empty()) {
		__common.log(ERR, "There is no symbols in DB");
		return false;
	}

	for (const string& symbol : __common.symbols)
	{	
		__common.log_fmt(INFO, "(symbol:%s)", symbol.c_str());
	}		
