...
void CAPIClient::threadfunc_parse_deploy()
{
	while (!m_thrdFlag.is_stopped())
	{
		...
		if (!m_parsingBuffer.empty())
		{
			...
			{
				std::string packet = m_parsingBuffer.substr(0, pos);										// \r\r 직전까지 추출
				m_parsingBuffer.erase(0, pos + 2);															// 처리된 부분 삭제 (2는 "\r\n" 길이)

				size_t len2 = m_parsingBuffer.size();


				__MAX::TData* pDeliver = gCommon.memPool.Alloc();
				pDeliver->init();
				strcpy(pDeliver->d, packet.c_str());

				if (packet.find(__MAX::CD_SISE) != std::string::npos)
				{
					PostThreadMessage(m_idTick, __MAX::WM_MARKET_DATA, (WPARAM)0, (LPARAM)(pDeliver));
				}
				else if (packet.find(__MAX::CD_HOGA) != std::string::npos)
				{
					PostThreadMessage(m_idQuote, __MAX::WM_MARKET_DATA, (WPARAM)0, (LPARAM)(pDeliver));
				}
			}
		}
	}
