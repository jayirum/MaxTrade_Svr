...
CBOTDbSaver __dbworks;


CBOTDbSaver::CBOTDbSaver()
{
...
bool CBOTDbSaver::connect()
{
	m_pDB = new CDBConnector();
	return m_pDB->connect_db();
}

bool CBOTDbSaver::save_chartdata(const DataUnitPtr& api)
{
	char zQ[1024];
	sprintf(zQ, 
		"SP_CHART_F_SAVE "
...
std::set<int> CBOTDbSaver::load_timeframes()
{
	std::set<int> set_tf{};
...
std::vector<shared_ptr<TSymbol>> CBOTDbSaver::load_symbols()
{
