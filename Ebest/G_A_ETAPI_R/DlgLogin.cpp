...
BOOL CDlgLogin::GetApiLoginInfo()
{
	...
	sprintf(zQ, "SELECT TOP 1 API_ID, API_PWD, API_CRTF_PWD FROM API_MST WHERE API_USE_YN='Y' AND ACNT_TP='%c' AND API_NM='이트레이드API'",
		DEF_ACNT_TP);
...
BOOL CDlgLogin::Login()
{
	...
	LOGGING(LOGTP_SUCC, TRUE, "API 로그인 시도(API ID:%s)(API PWD:%s)(Cert Pwd:%s)",
		m_zApiID, m_zdApiPwd, m_zCertPwd);
	...
	if( bResult == FALSE )
	{
		...
		LOGGING(LOGTP_ERR, FALSE, "API 로그인 에러:%s", (LPCSTR)strMsg);
...
LRESULT CDlgLogin::OnLogin( WPARAM wParam, LPARAM lParam )
{
	...
	if( atoi( pszCode ) == 0 )
	{
		LOGGING(LOGTP_SUCC, TRUE, "API 로그인 성공 수신");
	...
	{
		LOGGING(LOGTP_ERR, TRUE, "API 로그인 실패 수신");
