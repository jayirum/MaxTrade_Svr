...
#define EXE_VERSION		"v4.0.0"	// IOCP 서버 탑재
...
struct TAPIData {
	...
	std::string	dt_kor;
	std::string	tm_kor_hms;
	std::string	o, h, l, c, v;
	
	void set(std::string& s, 
			std::string& tf, 
			char* candle_kor_ymd_hms, // yyyymmdd_hhmm
			...
			)
	{
		...
		char z[32]={0};
		sprintf(z,"%.8s", candle_kor_ymd_hms);	dt_kor = std::string(z);
		sprintf(z,"%.6s", candle_kor_ymd_hms +8);tm_kor_hms = std::string(z);
